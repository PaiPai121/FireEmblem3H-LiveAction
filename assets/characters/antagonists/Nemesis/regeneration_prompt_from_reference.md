# Nemesis Reference-Based Regeneration Prompt

Use case: project character asset, live-action adaptation

Official reference image, local only:

`assets/characters/antagonists/Nemesis/references/official_local/concept/fireemblemwiki_main.png`

Current generated candidate:

`assets/characters/antagonists/Nemesis/selected.png`

Primary request:

Create a new photorealistic live-action adaptation of Nemesis from Fire Emblem: Three Houses. Use the official reference image as the source of truth for hair shape, costume silhouette, color blocking, accessories, and faction identity. Use the current generated candidate only as a quality baseline for cinematic realism, not as the design authority.
...
- Keep age and era appropriate for the `concept` version.
